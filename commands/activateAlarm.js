                ...
                if (!cron_list[alarm_to_silence].running) {
                    try {
                        if (alarm_to_silence.includes(private_flag)) {
                            await Private_alarm_model.update(
                        ...
                        msg.channel.send(`${alarm_to_silence} is now active again.`);
                    } catch (e) {
                        logging.logger.info(`Error re-activating alarm with id:${alarm_to_silence}... Please try again later!`);
                        logging.logger.error(e);
                        msg.channel.send(`Error re-activating alarm with id:${alarm_to_silence}... Please try again later!`);
                    }
                } else {
