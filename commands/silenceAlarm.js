    ...
    async execute(msg, args, client, cron, cron_list, mongoose) {
        if (args.length >= 1) {
            ...
            if (utility_functions.isOtaAlarm(alarm_to_silence)) {
               await msg.channel.send('Silence feature is not available for oneTimeAlarms, if you wish to silence a oneTimeAlarm just delete it with `$deleteAlarm ' + alarm_to_silence + '`');
                return;
            }
            ...
            if (!utility_functions.isPublicAlarm(alarm_to_silence) && msg.channel.type === 'dm') {
               await msg.channel.send('Impossible to silence a public alarm in DMs');
                return;
            }
            else {
                if (cron_list[alarm_to_silence] !== undefined) {
                    if (utility_functions.isPublicAlarm(alarm_to_silence)) {
                        ...
                        if (!b) {
                           await msg.channel.send('Alarm cannot be silenced. Check if it is setup in this server...');
                            return;
                        }
                    }
                                ...
                                if (timeout_date > current_date) {
                                    let c = new cron(timeout_date, async () => {
                                        if (!cron_list[alarm_to_silence].running) {
                                           ...
                                           await msg.channel.send(`Alarm with id ${alarm_to_silence} is now active again!`);
                                        }
                                    });
                                    c.start();
                                   await msg.channel.send(`Alarm will be activated again in ${timeout}`);
                                }
                                else {
                                   await msg.channel.send('The date you have provided has already happened...');
                                    return;
                                }
                            } else {
                               await msg.channel.send('The date you entered is invalid');
                                return;
                            }
                        }
                        try {
                           ...
                           await msg.channel.send(`${alarm_to_silence} is now silent.`);
                        } catch (e) {
                            logging.logger.info(`Error silencing alarm with id:${alarm_to_silence}... Please try again later!`);
                            logging.logger.error(e);
                           await msg.channel.send(`Error silencing alarm with id:${alarm_to_silence}... Please try again later!`);
                        }
                    } else {
                       await msg.channel.send(`Alarm with id ${alarm_to_silence} has already been silenced.\nTo see the status your alarms type: ${auth.prefix}myalarms`);
                    }
                }
                else {
                   await msg.channel.send(`Impossible to silence alarm with id ${alarm_to_silence}.\nPlease try again later.`);
                }
            }
        } else {
