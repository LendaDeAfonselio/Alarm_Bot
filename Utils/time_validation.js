...
async function validate_alarm_parameters(interaction, cron_stg, message_stg) {
    let cron_params = cron_stg.split(' ');
    if (message_stg.length <= 3) {
        await interaction.reply({ content: 'The message is to short! Use at least 3 characters', ephemeral: true });
        return false;
    }

    if (message_stg.length > 350) { // message is too long
        await interaction.reply({ content: 'The message is too long, please trim it down!', ephemeral: true });
        return false;
    }

    if (cron_params.length < 5) {
        await interaction.reply({ content: 'Not enough parameters were passed, try `$alarmHelp` for more information!', ephemeral: true });
        return false;
    }
    let mins = cron_params[0];
    if (!isAValidRangeGroupOrNumber(mins, 0, 59)) {
        await interaction.reply({ content: 'The minute parameter is invalid. This value must be between 0 and 59.\nTry `/alarmHelp` for more information!', ephemeral: true });
        return false;
    }
    if (small_time_interval(mins)) {
        await interaction.reply({ content: 'The minute parameter you sent is susceptible to spam. Only groups of 5 members or less and digits are allowed to avoid spam.', ephemeral: true });
        return false;
    }

    let hours = cron_params[1];
    if (!isAValidRangeGroupOrNumber(hours, 0, 23)) {
        await interaction.reply({ content: 'The hour parameter is invalid! This value must be between 0 and 23.\nTry `$alarmHelp` for more information!', ephemeral: true });
        return false;
    }

    let month_day = cron_params[2];
    if (!isAValidRangeGroupOrNumber(month_day, 1, 31)) {
        await interaction.reply({ content: 'The day of the month parameter is invalid! This value must be between 0 and 23.\nTry `$alarmHelp` for more information!', ephemeral: true });
        return false;
    }

    let month = cron_params[3];
    if (!isAValidRangeGroupOrNumber(month, 0, 11)) {
        await interaction.reply({ content: 'The month parameter is invalid! This value must be between 0 and 11.\nTry `$alarmHelp` for more information!', ephemeral: true });
        return false;
    }

    let weekday = cron_params[4];
    if (!isAValidRangeGroupOrNumber(weekday, 0, 6)) {
        await interaction.reply({ content: 'The weekday parameter is invalid! This value must be between 0 (Sunday) and 6 (Saturday).\nTry `$alarmHelp` for more information!', ephemeral: true });
        return false;
    }
